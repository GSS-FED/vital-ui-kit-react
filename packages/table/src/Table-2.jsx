...
export default class Table extends React.PureComponent<Props, State> {
    ...
    return (
      ...
      >
        <MultiGrid
          ...
          cellRenderer={props =>
            this._createGrid({ columns, ...props })
          }
          ...
          fixedRowCount={1}
          fixedColumnCount={
            columns.filter(column => column.props.fixed === true)
              .length
          }
  ...
  _createColumn = ({
    ...
    const title =
      typeof renderedCell === 'string' ? renderedCell : null;
  ...
  _createGrid: CellRenderer = ({
    ...
    if (rowIndex === 0) {
      const { label } = columns[columnIndex].props;
  ...
  _onHoverCell = ({ rowIndex, columnIndex }) => {
    this.setState({
